...
#else
  #error "no define platform ,ch579(cortex m0) or ch573(risc-v)"
#endif
}
...
void on_bleuartServiceEvt(uint16_t connection_handle,ble_uart_evt_t  *p_evt) {
    switch(p_evt->type) {
        ...
        case BLE_UART_EVT_BLE_DATA_RECIEVED:
            ...
            //tmos_start_task( Peripheral_TaskID, UART_TO_BLE_SEND_EVT,2);
            to_write_length = 3;
            app_drv_fifo_write(&app_uart_rx_fifo,(uint8_t *)"OK", &to_write_length );
            tmos_start_task( Peripheral_TaskID, UART_TO_BLE_SEND_EVT,2);
            ...
            //check cmd
            //if(strncmp((uint8_t *)p_evt->data.p_data,"open",4)==0)   OPEN_KEY=1;
            //if(strncmp((uint8_t *)p_evt->data.p_data,"close",4)==0)  CLOSE_KEY=1;
            if((uint8_t *)p_evt->data.p_data[0]=='open') OPEN_KEY=1;
            if((uint8_t *)p_evt->data.p_data[0]=='close') CLOSE_KEY=1;
...
void sendadc(){
...

    {
        ...
        memcpy(to_test_buffer,"ADC",4);
        read_length = 4;
        //app_drv_fifo_write(&app_uart_tx_fifo,to_test_buffer,&read_length);
        //read_length = 0;
        tmos_start_task( Peripheral_TaskID, UART_TO_BLE_SEND_EVT,2);
    }*/
    int to_write_length = strlen(to_test_buffer);
    app_drv_fifo_write(&app_uart_rx_fifo,to_test_buffer, &to_write_length );
    tmos_start_task( Peripheral_TaskID, UART_TO_BLE_SEND_EVT,2);
}
          ...
          case SEND_TO_BLE_TO_SEND:
            ...
            if(!ble_uart_notify_is_ready(peripheralConnList.connHandle)){
             ...
             if(app_drv_fifo_length(&app_uart_rx_fifo)>= read_length){
                  ...
                  //kkk
                  //result = app_drv_fifo_write(&app_uart_tx_fifo,to_test_buffer,&read_length);
